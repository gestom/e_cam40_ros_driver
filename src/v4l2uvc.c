...
int init_videoIn(struct vdIn *vd, char *device, int *width, int *height, float fps, int format, int grabmethod)
{
    ...
    *height = vd->height;
    ...
    //vd->framesizeIn = (vd->width * vd->height *4);
    switch (vd->formatIn) {
	    ...
	    case V4L2_PIX_FMT_Y16:
		    vd->framebuffer =  (unsigned char *) calloc(1, (size_t) vd->width * vd->height*4);
		    break;
	    default:
...
static int init_v4l2(struct vdIn *vd)
{
	...
	for(i = 0; i < ARRAY_SIZE(device_formats) && device_formats[i]; i++) {
		...
		if(device_formats[i] == V4L2_PIX_FMT_Y16 ||device_formats[i] == V4L2_PIX_FMT_MJPEG || device_formats[i] == V4L2_PIX_FMT_YUYV)
			fallback_format = i;
	}
